...
class KnowledgeIngestor:
    ...
    def ingest_udemy_curriculum(self) -> Dict[str, Any]:
        ...
        if not os.path.exists(UDEMY_DIR):
            print(f"Warning: Udemy directory not found at {UDEMY_DIR}")
            return curriculum
        ...
        print(f"Found {len(section_dirs)} Udemy sections. Parsing subtitles...")
        ...
        print(f"Ingested {len(curriculum)} chapters into {curriculum_file}")
        return curriculum
    ...
    def ingest_reference_books(self, max_pages_per_book: int = 50):
        ...
        for fname, (title, cat) in book_mappings.items():
            ...
            print(f"Ingesting reference book: {title}...")
            try:
    ...
    def ingest_stories(self):
        ...
        print(f"Ingesting {len(story_files)} story collections...")
        for spath in story_files:
            ...
            try:
                ...
                print(f"Error reading story {bname}: {e}")
    ...
    def generate_verified_quiz_banks(self):
        ...
        print(f"Generated {len(ch1_questions)} pre-verified questions in {ch1_bank_path}")
        ...
        for c_idx in range(2, 19):
            ...
            if not os.path.exists(bank_path):
                ...
                print(f"Generated {len(ch_qs)} pre-verified questions in {bank_path}")
    ...
    def run_full_ingestion(self):
        ...
        print("=== Step 1: Ingesting Udemy Curriculum ===")
        self.ingest_udemy_curriculum()

        print("\n=== Step 2: Ingesting Reference Books ===")
        self.ingest_reference_books(max_pages_per_book=40)

        print("\n=== Step 3: Ingesting Story Scenarios ===")
        self.ingest_stories()

        print("\n=== Step 4: Generating Pre-Verified 40-50 Question Quiz Banks ===")
        self.generate_verified_quiz_banks()

        counts = self.rag.count_chunks()
        print("\n=== Ingestion Complete! Summary of RAG Chunks ===")
...
if __name__ == "__main__":
    ...
    if args.inspect:
        ...
        print("Current RAG Store Summary:")
        for stype, cnt in counts.items():
            print(f"  {stype}: {cnt} chunks")
        banks = glob.glob(os.path.join(BANKS_DIR, "*.json"))
        print(f"Quiz banks available: {len(banks)} files")
    else:
