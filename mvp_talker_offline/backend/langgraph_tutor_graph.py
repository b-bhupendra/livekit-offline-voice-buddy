...
from structured_logger import system_logger, rag_logger

WORKSPACE_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DATA_DIR = os.path.join(WORKSPACE_ROOT, "data")
DB_PATH = os.path.join(DATA_DIR, "memory.db")
CHECKPOINT_DB_PATH = os.path.join(DATA_DIR, "langgraph_checkpoints.db")
...
class LangGraphTutorEngine:
    ...
    def _init_knowledge_store(self):
        ...
        rag_logger.info("Initializing LangGraph Local Knowledge Store from memory.db and curriculum.json...")
        count_chunks = 0
        try:
            if os.path.exists(self.db_path):
                ...
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT id, text_content, source_title, section_title, chapter_idx
                    FROM rag_chunks
                """)
                rows = cursor.fetchall()
                for r in rows:
                    source = (r["source_title"] or "general_grammar").lower().replace(" ", "_")
                    section = (r["section_title"] or "general").lower().replace(" ", "_")
                    item_id = str(r["id"])
                    data = {
                        "text": r["text_content"],
