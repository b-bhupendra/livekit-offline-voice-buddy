...
## 6. Resolved: the three open items, researched

These were left as open questions in the first draft of this plan. Each has a concrete, verified answer now.

### 6.1 Hardware budget for two-model parallelism

Ollama exposes two env vars that directly govern this, confirmed against current docs and independent benchmarking writeups:

- **`OLLAMA_MAX_LOADED_MODELS`** — how many distinct models can stay resident at once. Default is `3 × GPU count` (or `3` for CPU-only). This is already ≥2 by default on any single-GPU box, so nothing needs to be raised just to *permit* two models loading — the real constraint is VRAM, not this setting.
- **`OLLAMA_NUM_PARALLEL`** — concurrent requests per *loaded* model (default auto-selects 4 or 1 depending on available memory). Leave this at `1` for both the fast and authoring models in this project — you don't need concurrent requests to the same model, and raising it multiplies KV-cache memory for no benefit here.
- **GPU loading is all-or-nothing per model.** Ollama's docs confirm a new model must fully fit in *remaining* VRAM to load concurrently; there's no partial-offload sharing between two resident models. If the second model doesn't fit, Ollama queues the request and unloads an idle model to make room — which is exactly the multi-second stall the two-tier design exists to avoid, so under-provisioning silently defeats the whole architecture rather than erroring loudly.

**Concrete budget math** (plan against this before assuming it'll fit): weights ≈ file size on disk for a GGUF quant, plus 15–20% overhead for KV cache and CUDA runtime.

| Component | Approx. VRAM (Q4_K_M) |
|---|---|
| Faster-Whisper (`tiny.en`, int8) | ~1 GB |
| Kokoro ONNX | ~1 GB |
| Fast conversational model (`qwen2.5:3b`) | ~2–2.5 GB |
| **Tier 1 subtotal** | **~4–4.5 GB** |
| Authoring model (`qwen2.5:7b`) | ~4.5–5 GB |
| Authoring model (`qwen2.5:14b`, if used instead) | ~9 GB |

On an 8GB-class GPU, Tier 1 alone consumes over half the budget — there is no realistic way to keep a 7B+ authoring model resident *at the same time* without OOM or paging. This confirms (not just flags) that the earlier "operational strategy" recommendation is required, not optional, on that class of hardware:

- **During live calls:** only Tier 1 models may be resident. Set `OLLAMA_KEEP_ALIVE` short enough (or explicitly unload) that an idle authoring model doesn't linger in VRAM once a call starts.
- **During batch compilation:** Tier 1 is idle (no active call), so the authoring model gets the full GPU.
- **If you have 16GB+ VRAM or a second GPU:** true concurrent residency becomes viable — verify with `ollama ps` (shows currently loaded models and their VRAM) under real load before relying on it, rather than trusting the arithmetic alone.
- **The cloud-API-for-Tier-2-only fallback remains the simplest sidestep** if the local budget doesn't work out — it was correctly identified before, this research just confirms *why* it's often the pragmatic choice on consumer GPUs rather than a concession.

### 6.2 Ollama structured-output reliability — confirmed pattern, with the missing retry layer

`format=Schema.model_json_schema()` (equivalently, LangChain's `with_structured_output(..., method="json_schema")` from §3.2) is confirmed as Ollama's **officially recommended pattern**, working reliably "on Ollama 0.3.0 or newer," and multiple independent sources report it "alone kills [most] parse failures" for 7B+ models. So the mechanism in §3.2 is the right one — the gap in the original plan was not having a fallback for the edge cases that remain even with schema-constrained decoding. Add this defensive layer around every structured-output call in the authoring pipeline, not just §3.3's critique gate:

```python
import json, json_repair
from pydantic import ValidationError

def parse_structured(raw: str, model_cls: type[BaseModel]) -> BaseModel:
    cleaned = raw.strip()
    ...
    try:
        ...
        return model_cls.model_validate(json_repair.loads(cleaned))  # last-resort repair

def structured_authoring_call(model_cls, prompt, max_retries=3):
    last_error = None
    for attempt in range(max_retries):
        raw = structured_model.invoke(prompt if not last_error else f"{prompt}\n\nYour previous attempt failed validation: {last_error}. Fix exactly this and return the full corrected JSON.")
        try:
            return parse_structured(raw.content if hasattr(raw, "content") else raw, model_cls)
        except (ValidationError, json.JSONDecodeError) as e:
            last_error = str(e)  # feed the SPECIFIC field/error back, not a generic "try again"
    raise RuntimeError(f"structured authoring failed after {max_retries} attempts: {last_error}")
```
The specific detail worth keeping from research: retries that feed back the **exact validation error** ("`canvas_type`: expected one of [classifier, matrix, tree, flow], received 'particle_sort'") measurably outperform generic "that was wrong, try again" retries, because the model can act on a concrete diff. This is a small addition to §3.2/§3.3 — wrap every authoring call in this retry loop, and treat exhausted retries as a hard failure that the critique gate logs and skips (falls through to the "flagged for human review" state) rather than silently accepting malformed content.

One adjacent finding worth flagging so it isn't mistaken for the same problem: there's a well-documented Ollama/`qwen2.5-coder` issue where certain *client tools* (Cline specifically) fail to parse the model's tool-call JSON because the client's own parser expects a different wire format. That's a bug in that specific client's parsing logic, not in Ollama's structured-output or tool-calling support itself, and it doesn't apply here since this project drives Ollama through LangChain's `ChatOllama`, not through Cline — noted only so it isn't confused for evidence that qwen2.5 itself is unreliable at structured output, which the evidence above doesn't support.

### 6.3 `langgraph dev` — confirmed unresolved upstream, confirmed workaround is sufficient

This is a real, currently open GitHub issue (`langchain-ai/langgraph#5790`, no fix or documented flag as of this research): `langgraph dev` forcibly runs its own `langgraph_runtime_inmem` regardless of what checkpointer is compiled into the graph, and there is no command-line option to disable this. So there is no way to make `langgraph dev` itself respect `AsyncSqliteSaver` — that's confirmed, not just suspected.

The good news: this only matters if the project's iteration workflow uses the `langgraph dev` CLI server at all. Since this codebase runs the compiled graph directly from `agent.py` (as described in §3.1) rather than through that CLI, **the bug doesn't affect this project's actual runtime path** — it would only bite if someone later reaches for `langgraph dev` as a convenient way to iterate on the curriculum graph in isolation, gets an empty `checkpoint.db`, and burns time debugging a persistence bug that isn't in their code. Action item is exactly what was proposed before, now confirmed as the complete fix rather than a stopgap: a one-line note in the repo (e.g. a comment at the top of `curriculum_graph.py`) — *"Do not use `langgraph dev` to test persistence — it ignores compiled checkpointers (langgraph#5790). Invoke the compiled graph directly, as `agent.py` does."*
